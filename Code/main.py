...
connection = pymysql.connect(host="sql.haegelix.de", user="db_flunky", password="fsdhjkvgu245jhsdg", database="flunkyball")
...
class TeamLayoutFourGamers(Screen):
    ...
    #     popup.open()
    def showrules(self):
        Rules_Help_About.showrules(self)
...
class Game_Four_Players(Screen):
    ...
    def increment(self):
        ...
        if self.counter == 1:
            ...
            Game_Ten_Players.lastplayer(self)
            self.counter=0
    ...
    def changebutton_notDone(self, *args):
        self.ids["startbutton"].text = "Not Done?"
...
class Game_Ten_Players(Screen):
    ...
    def lastplayer(self):
        ...
        Clock.schedule_once(self.disableButton, 0.5)
        Clock.schedule_once(partial(self.labeltoblue, "Team2", -1), 9)
        Clock.schedule_once(self.changebutton_notDone, 9)
        Clock.schedule_once(self.changebutton, 10)
        Clock.schedule_once(self.enableButton, 10)
